...
import PatientCommunity from './PatientCommunity';
import PatientEmergency from './PatientEmergency';
import PatientNotifications from './PatientNotifications';
import PatientFaq from './PatientFaq';
...
function PatientProfile({ userId, email, initialName, onSaved }: { userId: string; email: string; initialName: string; onSaved: (name: string) => void }) {
  ...
  return <section className="rtp-card rtprofile-card" aria-labelledby="rtprofile-title">
    ...
    {loading ? <p role="status">Loading your profile…</p> : original && <form onSubmit={save} className="rtprofile-form">
      ...
      <label>Mobile number <span className="rtprofile-optional">(optional)</span><input type="tel" value={mobile} onChange={event => setMobile(event.target.value)} placeholder="+254712345678" autoComplete="tel" inputMode="tel" maxLength={16} disabled={saving}/><small>Use the international format beginning with + and your country code.</small></label>
      <div className="rtprofile-divider"><span>Read-only account information</span></div>
      <label>Email address<input type="email" value={email} disabled readOnly/></label>
...
export default function PatientPortal({ name, email, userId, onSignOut }: Props) {
  ...
  return <div className="rtp-shell">
    <aside className={`rtp-sidebar ${menuOpen ? 'rtp-sidebar-open' : ''}`}>
      ...
      <button type="button" className="rtp-identity" onClick={() => navigate('My Profile')} aria-label="Open my profile">
        <div className="rtp-avatar" aria-hidden="true">{displayName.slice(0, 1).toUpperCase()}</div><div><strong>{displayName}</strong><small>Patient account</small></div>
      </button>
      <nav aria-label="Patient navigation" className="rtp-nav">
        {links.map(({ label, Icon }) => <button key={label} type="button" className={`rtp-nav-item ${label === active ? 'rtp-active' : ''}`} aria-current={label === active ? 'page' : undefined} onClick={() => navigate(label)}><Icon size={19}/>{label}</button>)}
      </nav>
      <div className="rtp-sidebar-footer"><button type="button" className={`rtp-emergency ${active === 'Emergency' ? 'rtp-active' : ''}`} onClick={() => navigate('Emergency')}>✱ &nbsp; Report Emergency</button><button type="button" className="rtp-signout" onClick={() => void onSignOut()}><LogOut size={17}/>Sign out</button></div>
    </aside>

    <button type="button" className="rtp-mobile-backdrop" aria-label="Close patient navigation" tabIndex={-1} onClick={() => setMenuOpen(false)} hidden={!menuOpen}/>

    <div className="rtp-main"><header className="rtp-topbar"><button type="button" className="rtp-menu" aria-label="Toggle patient navigation" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X/> : <Menu/>}</button><strong>Patient Portal</strong>
      <div className="rtp-top-actions">
        <button type="button" className="rtp-icon-button" aria-label="Open notifications" onClick={() => { setNotificationsOpen(true); setFaqOpen(false); }}><Bell size={19}/></button>
        <button type="button" className="rtp-icon-button" aria-label="Open frequently asked questions" onClick={() => { setFaqOpen(true); setNotificationsOpen(false); }}><CircleHelp size={19}/></button>
      ...
      <main className="rtp-content"><div className="rtp-welcome"><div><h1>Jambo, {displayName.split(/\s+/)[0]}</h1><p>Welcome to RTTRACK. Your account is ready.</p></div><span className="rtp-status">Status: Account created</span></div>
        ...
        {active === 'My Profile' ? <PatientProfile userId={userId} email={email} initialName={displayName} onSaved={setDisplayName}/> :
          active === 'Connections' ? <ConnectionsPanel role="patient" userId={userId}/> :
          active === 'Treatment' ? <TreatmentTracker userId={userId}/> :
          active === 'Symptoms' ? <PatientSymptoms userId={userId} patientName={displayName}/> :
          active === 'Reminders' ? <PatientReminders/> :
          active === 'Education' ? <EducationHub/> :
          active === 'Insights' ? <PatientInsights userId={userId}/> :
          active === 'Community' ? <PatientCommunity onNavigate={navigate}/> :
          active === 'Emergency' ? <PatientEmergency onNavigate={navigate}/> :
          <><TreatmentSummary userId={userId} onViewTreatment={() => navigate('Treatment')}/>
            <div className="rtp-secondary-grid"><section className="rtp-card rtp-review"><h2><HeartPulse size={22}/> Medical Review</h2><p>No medical review scheduled</p><small>Your care team can update your treatment and appointment information.</small><button type="button" disabled>Add to Calendar</button></section><section className="rtp-card rtp-actions"><h2>Quick Actions</h2><div className="rtp-action-grid">{[{label:'Log Symptom',Icon:HeartPulse},{label:'View Schedule',Icon:CalendarDays},{label:'Ask Zora',Icon:Sprout},{label:'Learn Side Effects',Icon:BookOpen}].map(({label,Icon})=><button type="button" key={label} onClick={()=>{ if (label==='View Schedule' || label==='Log Symptom' || label==='Learn Side Effects') navigate(label === 'Log Symptom' ? 'Symptoms' : label === 'Learn Side Effects' ? 'Education' : 'Treatment'); else setZoraOpen(true); }}><span><Icon size={23}/></span>{label}</button>)}</div></section></div>
          </>}
        <footer className="rtp-footer"><strong>RTTRACK Kenya</strong><small>Signed in as {email}</small></footer>
      </main>
    </div>

    <PatientNotifications open={notificationsOpen} onClose={() => setNotificationsOpen(false)} onNavigate={navigate}/>
    <PatientFaq open={faqOpen} onClose={() => setFaqOpen(false)}/>
    <ZoraChat open={zoraOpen} onOpenChange={setZoraOpen} patientName={displayName} currentSection={active}/>
  </div>;
}
