...
export default function AdminReports(){
  ...
  function exportCsv(){
    ...
    trends.forEach(t=>lines.push(['Daily trend',t.day,'',t.doctor_reviews,t.education_actions,t.access_actions].map(csvCell).join(',')));
  ...
  return <section className="rtp-page"><header className="rtp-heading"><div><span className="eyebrow">ADMINISTRATION</span><h1>Reports</h1><p>Administrative workload and doctor/education counts. No patient-level data is exported.</p></div><div className="rtp-toolbar"><button type="button" className="outline" disabled={loading} onClick={()=>void refresh()}><RefreshCw size={16}/> Refresh</button><button type="button" className="primary" disabled={loading||!!error||!metrics.length} onClick={exportCsv}><Download size={16}/> Export CSV</button></div></header>
    ...
    <section className="rtp-trends"><h2>Daily administrative activity</h2><p>Doctor reviews, education actions and access changes by day.</p><div className="rtp-chart" role="img" aria-label="Daily administrative event totals">{trends.map(t=><div className="rtp-bar-item" key={t.day} title={`${t.day}: ${Number(t.doctor_reviews)+Number(t.education_actions)+Number(t.access_actions)} actions`}><div className="rtp-bar" style={{height:`${Math.max(2,(Number(t.doctor_reviews)+Number(t.education_actions)+Number(t.access_actions))/max*100)}%`}}/><span>{t.day.slice(5)}</span></div>)}</div></section>
    <div className="rtp-table-wrap"><table className="rtp-table"><thead><tr><th>Date</th><th>Doctor reviews</th><th>Education actions</th><th>Access changes</th></tr></thead><tbody>{trends.map(t=><tr key={t.day}><td>{t.day}</td><td>{t.doctor_reviews}</td><td>{t.education_actions}</td><td>{t.access_actions}</td></tr>)}</tbody></table></div></>}
  </section>;
}
